...
public class JT808ProtocolUtils {
	...
	public byte[] generateMsgHeader(String phone, int msgType, byte[] body, int msgBodyProps, int flowId,int msgTotalNum,int packageNum)
			throws Exception {
		ByteArrayOutputStream baos = null;
		try {
			baos = new ByteArrayOutputStream();
			//0.协议版本号
			baos.write(bitOperator.integerTo1Byte(128));
			// 1. 消息ID word(16)
			...
			//5.预留
			baos.write(bitOperator.integerTo1Byte(0));
			// 6.消息包封装项
			if(msgTotalNum>0 && packageNum>0){
				//消息总包数
				baos.write(bitOperator.integerTo2Bytes(msgTotalNum));
				//包序号
				baos.write(bitOperator.integerTo2Bytes(packageNum));
			}
			return baos.toByteArray();
		} finally {
			if (baos != null) {
